    ...
    Vue.component('file-item', {
        ...
        methods:{
            ...
            fileClass(file){
                ...
                if(file.file_type && file.file_type.substr(0,5)=='image'){
                    s.push('is-image');
                }else{
                    s.push('not-image');
                }
            ...
            getFileThumb(file){
                ...
                if(file.file_type && file.file_type.substr(0,5)=='video'){
                    return '<img src="/icon/007-video-file.png">';
                }
                if(file.file_type && (file.file_type.indexOf('x-zip-compressed')!== -1 || file.file_type.indexOf('/zip')!== -1)){
                    return '<img src="/icon/005-zip-2.png">';
                }
                if(file.file_type && file.file_type.indexOf('/pdf')!== -1 ){
                    return '<img src="/icon/002-pdf-file-format-symbol.png">';
                }

                if(file.file_type && (file.file_type.indexOf('/msword')!== -1 || file.file_type.indexOf('wordprocessingml')!== -1)){
                    return '<img src="/icon/010-word.png">';
                }
                if(file.file_type && (file.file_type.indexOf('spreadsheetml')!== -1  || file.file_type.indexOf('excel')!== -1)){
                    return '<img src="/icon/011-excel-file.png">';
                }
                if(file.file_type && file.file_type.indexOf('presentation')!== -1 ){
                    return '<img src="/icon/powerpoint.png">';
                }
                if(file.file_type && file.file_type.indexOf('audio/')!== -1 ){
            ...
            reloadLists(){
                ...
                $.ajax({
                    ...
                    success:function (json) {
                        ...
                        me.files = list || [];
                        me.total = json.total || me.files.length;
                        me.totalPage = json.totalPage || 1;
                        me.isLoading = false;
                        me.apiFinished = true;
                    }
                });
