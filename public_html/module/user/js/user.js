...
jQuery(function ($) {
    ...
    $('.upload-btn-wrapper').each(function () {
        ...
        $(document).on('change', '.btn-file :file', function (event) {
            ...
            $.ajax({
                ...
                beforeSend: function () {
                    input.trigger("bc-file-before-update")
                },
                success: function (data) {
                    if (data.status === 1) {
                        input.trigger("bc-file-update-success", data)
                    } else {
                        input.trigger("bc-file-update-error", data.message)
                    }
                },
                error: function (xhr) {
                    input.trigger("bc-file-update-error")
                },
                complete: function () {
                    input.trigger("bc-file-update-complete")
                },
                cache: false,
                contentType: false,
                processData: false
        ...
        container.find('.btn-file :file').on('bc-file-update-success', function (event, data) {
            console.log(data);
        ...
        container.find('.btn-file :file').on('bc-file-before-update', function () {
            container.find(".text-view").attr('value', container.find(".text-view").data("loading"));
        });
        container.find('.btn-file :file').on('bc-file-update-error', function (event, message) {
            if (message.length > 0) {
    ...
    $(".bc_user_profile .sidebar-menu .caret").click(function () {
        $(this).closest("li").toggleClass("active_child");
    });

    $(".bc_user_profile .bc-list-item .control-action .btn-danger").click(function () {
    ...
    $(".bc_user_profile .bc-list-item .control-action .btn-recovery").click(function () {
        var c = confirm($(this).data('confirm'));
        if(!c){
            return false;
        }
    ...
    $(".bc_user_profile .sidebar-menu .active_child").each(function () {
...
var vendorPayout = {
    saveAccounts:function (btn) {
        var parent = $(btn).closest('.bc-form');
...
jQuery(function($){
    ...
    $(document).ready(function() {
        $("#generate-password-new").on('click', function(e) {
            e.preventDefault();

            var $passwordInput = $("input[name='new-password']");
            var $passwordConfirm = $("input[name='new-password_confirmation']");
