...
<head>
    ...
    <link href="{{ asset('custom/css/app.custom.css') }}?_ver={{ config('app.asset_version') }}" rel="stylesheet">
    ...
    @if (setting_item_with_lang('enable_rtl'))
        <link href="{{ asset('themes/bc/dist/frontend/css/rtl.css') }}" rel="stylesheet">
    @endif
    @if (!is_demo_mode())
...
<body
    ...
    @endif
    <div class="bc_wrap">
        @if (auth()->user() and (!empty($adminbar_buttons) || is_admin()) and empty($_GET['preview']))
