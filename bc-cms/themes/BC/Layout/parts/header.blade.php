...
<ul id="user-dropdown-menu" class="dropdown-menu dropdown-menu-user text-left" style="z-index: 9999; display:none;">
    ...
    @endif
    @if (Auth::user()->hasPermission('blog_view'))
        <li class="@if (is_vendor() || is_baseAdmin()) menu-hr @endif"><a href="{{ route('blog.vendor.index') }}"><i
                    class="fa fa-newspaper-o"></i> {{ __('Blog') }}</a></li>
    @endif
    <li class="@if (is_vendor() || is_baseAdmin() || Auth::user()->hasPermission('blog_view')) menu-hr @endif">
