        ...
        <div class="d-flex align-items-center admin-bar-inner">
            <div class="d-flex align-items-center gap-3 admin-bar-left">
                @if(is_admin())
                    ...
                    <div class="dropdown">
                        <button class="btn btn-sm btn-secondary dropdown-toggle" style="font-size: 12px" type="button" data-toggle="dropdown" aria-expanded="false">
                            @include('Layout::global.icons',['icon'=>'setting'])
                            {{ __('Settings') }}
                        </button>
                        <div class="dropdown-menu" style="max-height: 500px; overflow-y: auto;">
                            @foreach(app(\Modules\Core\Models\Settings::class)::getSettingPages() as $id=>$group)
                ...
                @endif
                @if(!empty($adminbar_buttons))
                    @foreach($adminbar_buttons as $button)
                        <a
                            class="btn btn-sm btn-secondary font-weight-normal ml-1" style="font-size: 12px"
                            href="{{$button['url']}}"
                        >
                            @if(!empty($button['icon']))
                                @include('Layout::global.icons',['icon'=>$button['icon']])
                            @endif
                            <span class="ml-1">{{$button['label']}}</span>
                        </a>
                    @endforeach
                @endif
            </div>
