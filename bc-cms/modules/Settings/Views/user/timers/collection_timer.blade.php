@extends('Layout::user')
@section('content')
    <div class="container-fluid">
        <h2 class="title-bar">{{ $page_title }}</h2>
        @include('Settings::user.partials.hotel-required')
        @if(!empty($hotel_id))
        <div class="row">
            <div class="col-md-12 timer-width">
                <div class="panel">
                    <div class="panel-title">
                        <strong>{{ __('Collection timer settings') }}</strong>
                    </div>
                    <div class="panel-body">
                        <form action="{{ route('settings.vendor.collection-timer.store') }}" method="POST">
                            @csrf

                            <input type="hidden" name="type" value="{{ \Modules\Settings\Models\CollectionTimerSettings::TYPE_COLLECT }}">
                            <div class="form-group">
                                <label for="timer_hours">{{ __('Timer size (clock)') }} *</label>
                                <input type="number"
                                       class="form-control"
                                       id="timer_hours"
                                       name="timer_hours"
                                       value="{{ old('timer_hours', $timer_hours) }}"
                                       min="1"
                                       required>
                                <small class="form-text text-muted">{{ __('Set the collect timer size in hours (for example: 24)') }}</small>
                            </div>

                            <div class="form-group">
                                <button type="submit" class="btn btn-primary">{{ __('Save') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
        @endif
    </div>
@endsection
