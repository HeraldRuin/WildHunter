...
class Tour extends Bookable
{
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if (!empty($location)) {
            ...
            })->where("bc_tours.status", "publish")->with(['translation'])->count("bc_tours.id");
        }
        if (empty($number))
            return false;
        if ($number > 1) {
    ...
    {
        return $this->reviewClass::select(['id', 'title', 'content', 'rate_number', 'author_ip', 'status', 'created_at', 'object_author_id', 'author_id'])
    ...
    public function search($request)
    {
        ...
        foreach ($list_cat as $index => $cat) {
            $model_Tour->join('bc_tour_category as tc' . $index, function ($join) use ($cat, $index) {
                $join->on('tc' . $index . '.id', '=', 'bc_tours.category_id')
        ...
        if ($term_id = $request['term_id'] ?? "") {
            $model_Tour->join('bc_tour_term as tt1', function ($join) use ($term_id) {
                $join->on('tt1.tour_id', "bc_tours.id");
                $join->where('tt1.term_id', $term_id);
            });
        }

        if (!empty($request['attrs'])) {
            $model_Tour = $this->filterAttrs($model_Tour, $request['attrs'], 'bc_tour_term', 'tour_id');
        }
        ...
        if (!empty($request['is_featured'])) {
            $model_Tour->where('bc_tours.is_featured', 1);
        }
        if (!empty($request['custom_ids']) and !empty($ids = array_filter($request['custom_ids']))) {
            $model_Tour->whereIn("bc_tours.id", $ids);
