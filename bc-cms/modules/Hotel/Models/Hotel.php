...
class Hotel extends Bookable
{
    ...
    public function termsByAttributeInListingPage()
    {
        ...
        return $this->hasManyThrough($this->termClass, $this->hotelTermClass, 'target_id', 'id', 'id', 'term_id')->where('bc_terms.attr_id', $attribute)->with(['translation']);
    }
...
//            $booking->addMeta('extra_price', $extra_price);
    ...
    {
        return $this->reviewClass::select(['id', 'title', 'content', 'rate_number', 'author_ip', 'status', 'created_at', 'object_author_id', 'author_id'])->where('object_id', $this->id)->where('object_model', 'hotel')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('hotel_review_number_per_page', 5));
    }
...
//                'children'        => $room_children,
    ...
    public function search($request)
    {
        ...
        if (!empty($request['attrs'])) {
            $model_hotel = $this->filterAttrs($model_hotel, $request['attrs'], 'bc_hotel_term');
        }
        ...
        if (!empty($lat = $request["map_lat"] ?? "") and !empty($lgn = $request["map_lng"] ?? "") and !empty($request["map_place"] ?? "")) {
    ...
    public function getRelated()
    {
        ...
        $query = parent::query();
        $related_ids = $this->related_ids ? explode(',', $this->related_ids) : [];
        if (empty($related_ids)) {
            ...
            $query->whereIn('id', $related_ids);
            $query->orderByRaw('FIELD (' . $query->qualifyColumn("id") . ', ?) ASC', [implode(', ', $related_ids)]);
        }
